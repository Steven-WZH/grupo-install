    ...
    if ($create) {
        if (!$skip_message) {
            ...
            if (isset($attachments[0]) && $permission['generate_link_preview']) {
                preg_match_all('#\bhttps?://[^,\s()<>]+(?:\([\w\d]+\)|([^,[:punct:]\s]|/))#', $message, $links);

                if (isset($links[0])) {
                    if (isset($links[0][0])) {
                        include('fns/url_metadata/load.php');
                        $url_meta_data = url_metadata($links[0][0]);
            ...
            if (!empty($message)) {
                ...
                //$message = Emoji::text($message)->png()->toHtml(null, array(), Registry::load('config')->site_url);
            }

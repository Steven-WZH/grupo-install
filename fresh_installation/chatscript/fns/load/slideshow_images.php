...
if (role(['permissions' => ['super_privileges' => 'slideshows']])) {
    ...
    if (isset($data["slideshow"]) && !empty($data["slideshow"])) {
        ...
        if (!empty($data["slideshow"])) {
            ...
            $output['multiple_delete'] = new stdClass();
            $output['multiple_delete']->title = Registry::load('strings')->delete;
            $output['multiple_delete']->attributes['class'] = 'ask_confirmation';
            $output['multiple_delete']->attributes['data-remove'] = 'slideshow_images';
            $output['multiple_delete']->attributes['data-slideshow'] = $data['slideshow'];
            $output['multiple_delete']->attributes['multi_select'] = 'slideshow_image';
            $output['multiple_delete']->attributes['submit_button'] = Registry::load('strings')->yes;
            $output['multiple_delete']->attributes['cancel_button'] = Registry::load('strings')->no;
            $output['multiple_delete']->attributes['confirmation'] = Registry::load('strings')->confirm_action;
